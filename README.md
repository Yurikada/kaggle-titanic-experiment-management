...
| 木の深さ | 1〜10と制限なし | 深さ3 |
| モデル族 | 決定木 / ロジスティック回帰 / ランダムフォレスト | **ランダムフォレストへ変更** |

最初の4つは「変更に足る根拠なし」で終わりました。5つ目でランダムフォレストが決定木を
4.4 SE 上回り、**5比較で唯一の採用**になっています。
...
`17_random_forest_submission.py` です。

### 確定した構成と、公開スコアとの食い違い

- 特徴量8列、`Age` は全体中央値補完
- ランダムフォレスト `max_depth=5`, `max_features='sqrt'`, `n_estimators=100`

交差検証ではランダムフォレストが決定木を `+0.0071` 上回りましたが、**公開スコアでは
`-0.0024` と逆になりました**（0.77751 対 0.77990、418行中1行の差）。両者の差は公開スコアの
標準誤差 `0.020` の内側で、1回の提出では判定できません。採用判定は書き換えていません。

なお公開スコアは test 418行すべてで計算されます。3つのスコアがいずれも418分の整数
（326 / 325 / 310）になることから確認しました。当初は半分程度と仮定していましたが、
それは確認していない推測でした。
