...
NOTEBOOK_PATH = (
    ROOT / "uncertainty_publish" / "titanic_uncertainty_and_error_structure.ipynb"
)
REVIEW_DIR = ROOT / "uncertainty_review"
...
submission_path = (
    ROOT / "uncertainty_publish" / "outputs" / "submission_uncertainty_error.csv"
)
if not submission_path.exists():
    raise FileNotFoundError(
        f"The notebook did not produce {submission_path}."
    )
...
assert len(image_outputs) >= 7, "Expected visual outputs were not rendered."
...
for image_number, output in enumerate(image_outputs, start=1):
    image_path = REVIEW_DIR / f"uncertainty_output_{image_number:02d}.png"
    image_path.write_bytes(base64.b64decode(output["data"]["image/png"]))
...
print("PNG outputs:", len(image_outputs))
